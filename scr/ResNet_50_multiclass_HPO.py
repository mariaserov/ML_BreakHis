...
def create_model(weight_decay=0.0):
    ...
    output = layers.Dense(8, activation='softmax')(x)  # Adjust classes if needed
...
# 8. Train
# ==========================
os.makedirs("models_multiclass", exist_ok=True)
os.makedirs("logs_multiclass", exist_ok=True)
os.makedirs("plots_multiclass", exist_ok=True)

config_name = f"multiclass_job{job_index}_bs{config['batch_size']}_lr{config['learning_rate']}_{config['optimizer']}_wd{config['weight_decay']}"

callbacks = [
    EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True),
    ModelCheckpoint(f"models_multiclass/{config_name}.h5", save_best_only=True)
]
...
pd.DataFrame(history.history).to_csv(f"logs_multiclass/history_{config_name}.csv", index=False)
...
plt.tight_layout()
plt.savefig(f"plots_multiclass/plot_{config_name}.pdf")
plt.close()